...
const assignApplicanttointerviewer = async (req, res) => {
  const { interviewer_id, applicant_uuid, time_of_interview, comment } =
    req.body;
  ...
  try {
    ...
    const [insertOrUpdateResult] = await db.query(insertOrUpdateQuery, [
      interviewer_id,
      applicant_uuid,
      formattedDate,
    ]);
...
`;
      const [updateStatusResult] = await db.query(updateStatusQuery, [
        "moved to Interview",
        comment,
        applicant_uuid,
      ]);
      ...
      res
        .status(500)
        .json({ message: "Failed to assign applicant to interviewer." });
    }
  } catch (error) {
...
const assignApplicanttohr = async (req, res) => {
  ...
  try {
    ...
    // Execute the query
    const [insertOrUpdateResult] = await db.query(sqlQuery, [
      hr_id,
      applicant_uuid,
      formattedDate,
    ]);
...
const assigntoTrainer = async (req, res) => {
  ...
  try {
    ...
    if (insertOrUpdateResult.affectedRows > 0) {
      ...
      res
        .status(500)
        .json({ message: "Failed to assign applicant to trainer." });
    }
  } catch (error) {
