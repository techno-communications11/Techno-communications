...
function Hired() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const response = await axios.post(`${process.env.REACT_APP_API}/getdirecthiringdetails`, { assigned_user_id: userId },{withCredentials:true});
  ...
  const pendingApplicants = applicants.filter(applicant => 
     ['selected at Hr','mark_assigned'].includes(applicant.status) && applicant.hrevaluationData && applicant.hrevaluationData.length > 0
  );
  ...
  return (
    <div>
      ...
      {hasEvaluationData && pendingApplicants.length > 0 ? (
        ...
        <p>No pending applicants with evaluation data available.</p> // Only display this message when no pending applicants with evaluation data
      )}
    </div>
  );
}
